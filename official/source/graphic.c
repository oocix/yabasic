...
static Display *display;	/* X-Display */
...
{				/* open a Window */
  ...
  if (!backbit)
    {
      error (ERROR, "couldn't create backing pixmap");
      return;
    }
...
mywindowproc (HWND handle, unsigned msg, UINT wparam, DWORD lparam)
{
  ...
  switch (msg)
      ...
      if (wantakey)
	{
	  ...
	  y = HIWORD (lparam);
	  itransform (&x, &y);
	  sprintf (winkey, "MB%s+%d:%04d,%04d", detail, mod, x, y);
	  wantakey = FALSE;
	  SetEvent (gotkey);
	}
      return 0;
  ...
  if (printer)
    {
      if (clear)
	{
	  if (!(savedprnpen = SelectObject (printer, revprinterpen)))
	    error (ERROR, "could not select background printer pen");
	}
      else
	{
	  if (!(savedprnpen = SelectObject (printer, printerpen)))
	    error (ERROR, "could not select foreground printer pen");
	}
      return;
    }
}
...
{				/* change font */
  ...
  if (!XChangeGC (display, gc, GCFont, &xgcvalues))
    {
      sprintf (string, "Could not change font to '%s'", font);
      error (ERROR, string);
      return FALSE;
    }
  firsttext = TRUE;
  ...
  if (arg1 && arg2 && !align)
    {
      ...
      error (ERROR, "among the last two arguments");
      return;
    }
  if (!align)
    align = text_align;
  if (fontname && !change_font (fontname))
    return;
...
{				/* put rect into win */
  ...
  if (print_to_file)
    {
      error (ERROR, "Cannot bitblit to printer");
      return;
    }
  if (!winopened)
    {
  ...
  if (xe + we > 0 && ye + he > 0)
    {
      ...
      if (!bits)
	{
	  error (ERROR, "Couldn't get bits from window");
	  return;
	}
    }
#else /*  */
...
{				/* add one rgb pixel to bitstring */
  ...
  bitcount++;
  sprintf (pbits, "%02x%02x%02x\0", red, green, blue);
  pbits += 6;
}
